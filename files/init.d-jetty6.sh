...
# Source: http://wiki.apache.org/solr/SolrJetty
# Patched and status option added by Marji, www.morpht.com
...
usage()
{
    echo "Usage: $0 {start|stop|run|restart|check|supervise|status} [ CONFIGS ... ] "
    exit 1
}
...
case "$ACTION" in
  ...
  status)
        ...
        # start-stop-daemon - disabled as ubuntu < 12.04 does not recognise the -T option:
        # if [ "$START_STOP_DAEMON" = "1" ] && type start-stop-daemon > /dev/null 2>&1; then
        #  start-stop-daemon -T -p $JETTY_PID -d $JETTY_HOME -x $JAVA
        #  EXCODE=$?
        #  PID=$(cat "${JETTY_PID}" 2>/dev/null)
        # else
          PID=$(cat "${JETTY_PID}" 2>/dev/null)
        ...
        # fi
        case $EXCODE in
          3) echo "Jetty not running" ;;
          0) echo "Jetty is running (pid ${PID})" ;;
          1) echo "Jetty not running (pid file exists)" ;;
          *) echo "Jetty status unknown"
             EXCODE=4 ;;
        esac
        exit $EXCODE
        ;;
